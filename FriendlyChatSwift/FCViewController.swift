...
class FCViewController: UIViewController, UINavigationControllerDelegate {
	...
	fileprivate var _refUpdateHandle: DatabaseHandle!
    ...
    func configureDatabase() {
		...
		self.dataSource = self.messagesTable.bind(to: self.ref) { (tableView, indexPath, snapshot) in
			guard let cell = tableView.dequeueReusableCell(withIdentifier: "messageCell", for: indexPath) as? UITableViewCell else {
					fatalError("Cell not found")
			}
		
			self.message = snapshot
			let name = self.message.childSnapshot(forPath: Constants.MessageFields.name).value as? String ?? "none found"
			let text = self.message.childSnapshot(forPath: Constants.MessageFields.text).value as? String ?? "none found"
		...
		*/
		...
		_refAddHandle = ref.child("messages").observe(.childAdded){(snapshot: DataSnapshot) in
    ...
    deinit {
		...
		ref.child("messages").removeObserver(withHandle: 30)
    ...
    func configureRemoteConfig() {
		...
		//Remote config settings to enable developer mode
    ...
    func fetchConfig() {
		...
		remoteConfig.fetch(withExpirationDuration: expirationDuration) { (status, error) in
			if status == .success {
				...
				//Makes fetched enabled to be accessed for use
				self.remoteConfig.activateFetched()
				let friendlyMsgLength = self.remoteConfig["friendly_msg_length"]
				print("The source is \(friendlyMsgLength.source)")
    ...
    func signedInStatus(isSignedIn: Bool) {
        ...
        if (isSignedIn) {
			...
			// fetch most recent values of configured parameters
			fetchConfig()
        }
    }
    ...
    func sendPhotoMessage(photoData: Data) {
		...
		storageRef!.child(imagePath).putData(photoData, metadata: metadata) { (metadata, error) in
			...
			print("The path is \(metadata?.path)")
			print("The description is \(self.storageRef!.child((metadata?.path)!).description)")
...
extension FCViewController: UITableViewDelegate, UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
					// dequeue cell
			let cell: UITableViewCell! = self.messagesTable.dequeueReusableCell(withIdentifier: "messageCell", for: indexPath)
